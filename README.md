...
- **原生观感** — 执行结果渲染成类似「已思考」的折叠区块，例如「已调用工具（2次）」并逐条展示结果
...
- **确定性自动续读** — `local_file_read` 对超出单次读取上限的大文件，由扩展侧以代码循环确定性续读，不再依赖模型自觉续读，彻底消除「读到一半静默截断」的可靠性弱环节
- **Unicode 码点级定位** — 续读位置按 Unicode 码点（而非 UTF-16 单元）推进，emoji 与中文混排的文件也能逐窗口完整拼接、无字符丢失
- **宿主侧防 OOM** — 宿主按字节按需读取字符窗口，不再整文件读入内存，超大文件也不会撑爆内存
...
| 方向 | 主要变化 |
|------|----------|
| 内联智能体 | 最终回答改由 DeepSeek 页面原生渲染，mermaid/xychart 图表卡片、html/svg/xml 可运行代码块与原生预览面板直接展示；插件侧产物渲染流程移除。 |
| 推理过程 | 逐轮捕获并展示思考过程，完成后自动折叠为「思考 · 第 N 步」；最终回答完整展开，不再被折叠或截断。 |
| 稳定性 | 修复流式分帧可能丢失部分推理/正文增量、代码块跨帧折叠破坏围栏内容、代码运行结果在流式重渲染后丢失的问题；只有思考内容的回合不再重复提交。 |
...
1.13.1 是内联智能体体验打磨与对话导出稳定化版本：内联智能体输出渲染与原生 Markdown 一致、工具状态展示更真实，HTML 对话导出与下载链路更可靠。

| 方向 | 主要变化 |
|------|----------|
| 内联智能体 | 输出渲染改为与原生 Markdown 一致的紧凑块级排版，剥离工具调用后的多余空行自动折叠，标题层级对齐；工具执行状态如实展示，流式输出滚动更安全，暂停时显示中性页脚；图标改为 SVG 并统一类型与圆角体系，交互反馈更即时，键盘与读屏无障碍更好；启动失败不再无声。 |
| 对话导出 | HTML 导出包含输入/产出的统计信息，导出与下载链路更稳定。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

<details>
<summary>展开历史版本变更回顾（1.13.0 - 0.2.0）</summary>
...
| 方向 | 主要变化 |
|------|----------|
| 内联智能体 | 运行引擎升级，工具执行与授权链路更稳定；配置 DeepSeek 官方 API Key 后自动改用官方 API 后端运行（支持推理过程），未配置时仍使用页面会话链路。 |
| 本地 Skill | 本地文件夹导入改为索引导入（只存索引卡、内容用时读取），支持目录重定位后的重新关联与隐式自动激活；Skill 会话的 shell 工作目录自动限定在该 Skill 目录内。 |
| 技能生态 | 支持从 pi/agentskills.io 生态导入 SKILL.md 技能包，沿用现有本地导入流程。 |
| 性能 | 后台脚本结构性瘦身，i18n 资源按分区裁剪；存储改用浏览器原生 IndexedDB，启动与响应更快、内存更低。 |
| 对话导出 | 会话历史为空时导出明确失败；复制输出失败时显示具体原因。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

<details>
<summary>展开历史版本变更回顾（1.12.2 - 0.2.0）</summary>

### 1.12.2 变更回顾

1.12.2 是工具执行透明化与生产体验修复版本：工具失败不再黑箱、可一键导出诊断日志、复制完整输出、导出内容范围可选，并让内联智能体的续跑计数与实际步数一致。（长文产出自动保存入口已在新版本移除：最终交付物改由 DeepSeek 原生渲染呈现。）

| 方向 | 主要变化 |
|------|----------|
| 工具执行 | `shell_exec` 等工具失败时展示实际执行的命令与失败原因；授权被拒显示具体原因（授权关闭、过期、会话不匹配、工具禁用等）。设置 → 数据新增「导出诊断日志」，一键导出工具执行、授权与错误记录。 |
| 内联智能体 | 达到自动续跑上限暂停时，提示实际完成的步数，与「发送『继续』续跑」的提示口径一致。 |
| 长文产出 | （已移除）该版本曾将 Agent 长文产出自动保存为产物并提供下载入口；新版本中最终交付物改由 DeepSeek 原生渲染（图表卡片、可运行代码块），不再需要插件侧的保存与下载行。 |
| 复制输出 | 回复操作区新增「复制完整输出」按钮，一次复制 DeepSeek 的完整回复。 |
| 对话导出 | 导出 Markdown/HTML 支持「仅输入+产出」或「全部内容」范围选择，并记住上次选择。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |
...
| 方向 | 主要变化 |
|------|----------|
| 本地文件读取 | `local_file_read` 确定性自动续读：超过单次上限的大文件自动续读至末尾，emoji 与中文混排不再截断；超大文件按字符窗口按需读取，不再整文件载入内存。 |
| Firefox | 工具栏新增 DeepSeek++ 按钮，点击打开侧边栏，与 Chrome/Edge 的操作方式一致。 |
| 侧边栏 | 设置、能力、资料库的标签条溢出时显示左右箭头，隐藏的标签可一键滚动访问；键盘切换的标签自动滚入可视区。 |
| 内联智能体 | Agent 步骤时间线更清晰：过程文本与最终答案分离，工具调用按行展示成功/失败状态并可独立展开摘要；完成的步骤只折叠过程文本，工具行保持可扫读。 |
| 工具执行 | 移除手动确认模式，启用的工具自动执行，「禁用」成为唯一的退出方式。 |
| 可靠性与安全 | MCP 连接与读取加入超时保护与端点校验；工具解析改为线性时间（消除极端输入下的卡顿风险）；沙箱 HTML 内容钉扎资源来源并限制结果大小；联网读取仅允许 http(s) 地址。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |
...
| 方向 | 主要变化 |
|------|----------|
| Agent 运行 | 步骤界面更清晰并带全局运行指示；循环结束不再无声无息，重复步骤有重试上限；窗口类工具结果按窗口展示，续跑提示更准确。 |
| 原生搜索 | Agent 使用 DeepSeek 原生智能搜索时，不再触发扩展自带的联网搜索工具，避免重复搜索。 |
| 文件引用 | 新增可信目录与 @ 文件引用：在设置中选择本地目录（带扫描预算）后，可在聊天中引用目录内图片；越界路径会被忽略。 |
| 文件健壮性 | 图片 MIME 规范化避免同一图片重复上传；扩展上下文失效（如重载扩展后旧页面残留）时不再误报保存失败。 |
| 用量图表 | 30 天 Token 趋势图 X 轴只显示日期，标签不再挤出图表面板。 |
| MCP 兼容性 | MCP 客户端能力声明改为符合协议规范的有效值，与校验客户端能力的服务端连接更稳定。 |
| 页面加载 | 扩展页面不再输出 modulepreload 预加载链接，消除浏览器控制台中的相关噪音。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.9 变更回顾

1.11.9 聚焦聊天与 MCP 使用体验，修复编辑消息后的上下文注入、输出速度统计、输入法回车和 MCP 工具选择等问题，并让 DeepSeek 原生智能搜索与扩展工具更好地协同。

| 方向 | 主要变化 |
|------|----------|
| 编辑消息 | 编辑 DeepSeek 消息后，扩展注入的记忆、预设和工具提示会像新消息一样继续生效，不再丢失。 |
| 输出统计 | 模型输出速度只统计可见输出，不再把隐藏思考文本计入，避免数字虚高。 |
| 聊天输入 | 使用中文等输入法组合输入时，按回车选择候选词不会提前发送消息。 |
| MCP 兼容性 | 改善与使用标准流式格式的 MCP 服务的兼容性；手动模式下工具选择状态清晰可选，切换工具不会清空已发现的服务信息。 |
| 原生搜索 | 使用 DeepSeek 原生智能搜索时，普通对话和 Agent 续跑保持原生搜索，不再重复触发扩展自带的联网搜索。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.8 变更回顾

1.11.8 聚焦可选云同步可靠性，减少同步服务暂时繁忙时上传或下载个别数据文件失败的情况。

| 方向 | 主要变化 |
|------|----------|
| 云端同步 | 上传和下载同步数据时会有序处理文件，降低 WebDAV、Google Drive 或 OneDrive 暂时限制并发请求时的失败概率。 |
| 数据完整性 | 只有同一份同步数据完整下载并通过校验后，才会更新本地记忆、Skill、预设、项目和保存项。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.7 变更回顾

1.11.7 聚焦长对话恢复与 MCP 连接可靠性，减少刷新、页面切换或消息重新挂载后历史工具记录消失，以及已保存 MCP 服务状态不同步的问题。

| 方向 | 主要变化 |
|------|----------|
| 长对话恢复 | 刷新、路由切换或页面分段加载后，历史工具卡片、Artifact 结果和 Agent 续跑记录会在对应消息出现时恢复显示。 |
| 对话显示 | 恢复历史时保留用户实际输入的斜杠命令，避免把 Skill 的内部展开说明显示在对话里。 |
| MCP 连接状态 | 服务发现结果与健康状态同步保存；连接配置在刷新期间变化时，不再用旧结果覆盖最新配置。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.6 变更回顾

1.11.6 聚焦文件生成稳定性，避免 OfficeCLI 等 Skill 生成本地文档时，异常超大内容拖垮页面或在刷新后再次出现。

| 方向 | 主要变化 |
|------|----------|
| 文件生成防崩溃 | 对异常超大的工具输出增加硬性保护，超过安全范围后停止继续处理，避免页面因长串文件数据卡死。 |
| 刷新后恢复 | 清理历史记录中未完成、被拆分或格式带空白的工具输出，避免刷新页面后再次渲染残留的大段内容。 |
| Artifact 使用边界 | 可下载 Artifact 明确用于 UTF-8 文本和代码；Office、PDF、图片、压缩包等本地二进制文件应直接保留并返回本地路径，不再编码后塞入聊天。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.5 变更回顾

1.11.5 聚焦基础聊天、MCP 连接和 Shell Host 响应稳定性，减少请求格式变化、重复刷新或长时间本机工具调用带来的中断。

| 方向 | 主要变化 |
|------|----------|
| 基础聊天稳定性 | 当某类 DeepSeek 请求无法由扩展安全增强时，会直接按原请求继续发送，避免基础聊天被扩展中断。 |
| MCP 权限确认 | 在侧边栏连接需要网站权限的 MCP 服务时，会由当前用户操作直接发起权限确认，减少点击连接后无法正常授权的问题。 |
| MCP 工具刷新 | 同一服务的测试和刷新会按顺序执行；刷新失败时保留最近一次成功发现的工具，同时明确显示连接错误。 |
| Shell Host 响应 | 长时间运行本机工具、初始化 Windows 命令环境或打开文件夹选择器时，工具列表和其他控制请求仍可及时响应。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.4 变更回顾

1.11.4 聚焦工具授权、工具续跑和 Windows Firefox 本机工具连接的稳定性，减少页面切换、权限确认或多浏览器安装带来的中断。

| 方向 | 主要变化 |
|------|----------|
| 工具授权恢复 | 修复新建、切换或刷新 DeepSeek 会话后，已确认工具仍被错误拒绝的情况；「授权全部网站」可在侧边栏正常发起网站权限确认。 |
| 工具续跑 | 工具完成和会话结束事件会按正确顺序处理，减少联网搜索等工具执行一次后不继续生成的问题。 |
| MCP 工具恢复 | 服务状态更新后会保留已发现的工具，避免测试或刷新后工具列表意外变空。 |
| Windows Firefox Shell MCP | Windows 为每个浏览器保留独立的本机 Host 清单，Firefox 与 Chrome、Edge 或 Chromium 同时安装时不再互相覆盖。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.3 变更回顾

1.11.3 聚焦 MCP 工具恢复和页面运行稳定性，减少服务测试成功但工具列表为空、以及刷新后聊天或工具状态异常带来的中断。

| 方向 | 主要变化 |
|------|----------|
| MCP 工具恢复 | 修复服务测试成功后工具数仍显示为 0 的问题，已发现的工具列表会在状态更新后保持可用。 |
| 本地 Skill 导入 | Shell MCP 的文件夹选择工具可继续用于本地 Skill 导入，减少无法选择本机 Skill 文件夹的情况。 |
| 聊天与工具稳定性 | 改善页面刷新后的聊天与已授权工具连接稳定性，减少运行时状态相互干扰。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |

### 1.11.2 变更回顾

1.11.2 聚焦把升级后的基础聊天和既有 MCP 连接恢复到可预测状态，减少新对话首条消息失败、旧配置无法重新启用带来的中断。

| 方向 | 主要变化 |
|------|----------|
| 新对话稳定性 | 修复 DeepSeek 创建新会话时可能在首条消息前中断的问题，让普通对话可以正常开始并收到回复。 |
| 既有 MCP 恢复 | 升级后会识别并恢复仍然有效的 MCP 服务配置，减少因旧缓存而需要手动重建连接的情况。 |
| 会话与工具边界 | 工具授权继续绑定到用户当前的浏览器会话，避免页面切换影响正常聊天与已授权工具的连续使用。 |
| 权限变化 | Chrome、Edge 和 Firefox 均不新增浏览器权限。 |
